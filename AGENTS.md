...
| `remediation-queue` | `cli/remediation_queue.py` | Operator ack/resolve/snooze for decayed backlinks. Reads/writes `remediation.event` to events.db. `--list` (human table or `--json` JSONL), `--ack`, `--resolve`, `--snooze --days N`. `--fail-on-unresolved` exits 6. Exit 0 advisory. Plan: `docs/plans/2026-06-07-001-feat-backlink-remediation-queue-plan.md` |
| `replan-dead` | `cli/replan_dead.py` | Dead-link auto-replan: reads `link.rechecked` from events.db, selects recent deterministic dead links (host_gone/link_stripped), emits `plan-backlinks` seed JSONL. `--days N` (default 7), `--min-gap M` (default 3). Exit 0 advisory. Plan: `docs/plans/2026-06-07-002-feat-replan-dead-pipeline-plan.md` |
| `quality-gate` | `cli/quality_gate.py` | Pre-publish quality gate: reads seed JSONL on stdin, checks anchor density (`--max-density`) and content uniqueness (`--max-similarity`) against events.db. Optional LLM scoring (`--quality-llm`). Passes rows through on stdout; blocked rows logged to stderr. Exit 0 advisory. Plan: `docs/plans/2026-06-07-003-feat-quality-gate-plan.md` |
